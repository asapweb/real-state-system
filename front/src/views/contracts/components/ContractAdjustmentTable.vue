<template>
    ...
    <v-card-text>
      ...
      >
        ...
        <template #item.actions="{ item }">
          <v-icon class="me-2" size="small" @click="openDialog(item)" title="Editar ajuste">mdi-pencil</v-icon>
          <v-icon size="small" @click="confirmDelete(item)" title="Eliminar ajuste">mdi-delete</v-icon>
        </template>
      </v-data-table-server>
    </v-card-text>
  </v-card>
  ...
  <v-dialog v-model="dialogDelete" max-width="500px">
    <v-card>
      ...
      <v-card-text>
        ¿Estás seguro de que querés eliminar este ajuste?
      </v-card-text>
      <v-card-actions>
        <v-spacer />
...
const deleteAdjustment = async () => {
  try {
    await axios.delete(`/api/contracts/${props.contractId}/adjustments/${adjustmentToDelete.value.id}`);
