<template>
  <v-card>
    <v-card-text>
      ...
      <div v-else-if="loadingRoles">Cargando roles...</div>
      <div v-else-if="errorRoles">Error al cargar los roles.</div>
      <div v-else>No hay roles disponibles.</div>
    </v-card-text>
  </v-card>
</template>
...
const updateUserRoles = async (newRoles) => {
  try {
    const response = await axios.put(`/api/users/${props.userId}/roles`, {
