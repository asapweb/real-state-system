  ...
  <!-- Permisos agrupados -->
            <template v-for="(permissions, groupName) in groupedPermissions" :key="groupName">
              ...
              <template #label>
                <div>
                  <div class="font-weight-medium">Seleccionar todos</div>
                <div class="text-caption text-grey">Marcar o desmarcar todos los permisos de {{ groupName }}</div>
                </div>
              </template></v-checkbox>
            </div>
...
const updateRolePermissions = async () => {
  try {
    ...
    const response = await axios.put(`/api/roles/${roleId.value}/permissions`, { permissions: selectedPermissions.value });
