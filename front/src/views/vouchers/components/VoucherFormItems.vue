  ...
  <v-table striped="even" class="no-borders">
    <thead>
      <tr>
        ...
        <th class=" font-weight-bold text-start">Tipo IVA</th>
        <th class=" font-weight-bold text-end ">Subtotal</th>
        <th class=" font-weight-bold text-end ">IVA</th>
    ...
    <tbody>
      <tr>
        <td colspan="7" v-if="form.items.length === 0 && !showCreationRow">
      ...
      <tr v-for="(item, index) in form.items" :key="index">
        ...
        <td class="text-end">{{ calculateItemVat(item).toFixed(2) }}</td>
        <td class="text-end">{{ calculateItemTotal(item).toFixed(2) }}</td>
      ...
      <tr v-if="showCreationRow">
        ...
        <td class="text-end">{{ calculateItemVat(newItem).toFixed(2) }}</td>
        <td class="text-end">{{ calculateItemTotal(newItem).toFixed(2) }}</td>
