...
const props = defineProps({
  expenseId: { type: Number, required: true }, // ✅ Para contract-expense
})
...
const loadCategories = async () => {
  const { data } = await axios.get(`/api/attachment-categories/all?context=contract-expense`)
...
const loadAttachments = async () => {
  loading.value = true
  try {
    const { data } = await axios.get(`/api/attachments/contract-expense/${props.expenseId}`, {
      params: {
        search: search.value,
...
const uploadFile = async () => {
  ...
  try {
    await axios.post(`/api/attachments/contract-expense/${props.expenseId}`, formData)
