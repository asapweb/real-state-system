    ...
    <v-card-text>
      ...
      >
        ...
        <template #item.actions="{ item }">
          <v-icon size="small"  class="me-2" @click="editOwner(item)" title="Editar">mdi-pencil</v-icon>
...
const handleSave = async (formData) => {
  ...
  try {
    if (selectedOwner.value && selectedOwner.value.id) {
      await axios.put(`/api/properties/${props.propertyId}/owners/${selectedOwner.value.id}`, formData);
    } else {
